...
void romDisplayInfo ( struct gba_header *gbah )
{
...
main ( int argc, char *argv [] )
{
  ...
  if ( operation == "read" )
  {
    ...
    else
    {
      ...
      for ( u_int32_t l = block_offset; l < ( block_offset + block_count ); ++l )
      {
        ...
        t.readROM ( h, ( l * BLOCK_SIZE ), BLOCK_SIZE, buf );
  ...
  else if ( operation == "write" )
  {
    ...
    else
    {
      ...
      for ( u_int32_t l = block_offset; l < ( block_offset + block_count ); ++l )
      {
        ...
        t.writeROM ( h, ( l * BLOCK_SIZE ), buf, BLOCK_SIZE );
  ...
  else if ( operation == "readram" )
  {
    ...
    else
    {
      ...
      for ( u_int32_t l = block_offset; l < ( block_offset + block_count ); ++l )
      {
        ...
        t.readRAM ( h, ( l * RAM_BLOCK_SIZE ), buf, RAM_BLOCK_SIZE );
  ...
  else if ( operation == "writeram" )
  {
    ...
    else
    {
      ...
      for ( u_int32_t l = block_offset; l < ( block_offset + block_count ); ++l )
      {
        ...
        t.writeRAM ( h, ( l * RAM_BLOCK_SIZE ), buf, RAM_BLOCK_SIZE );
  ...
  else if ( operation == "info" )
  {
    ...
    if ( filename == "" )
    {
      ...
      t.readROM ( h, 0, 192, ( ez_byte_t* ) gbah );
